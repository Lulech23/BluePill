...
# Profile path
$path = "$env:WinDir\System32\spool\drivers\color\AyaNeo.icc"
...
if ($task.contains("Install")) {
    ...
    if (Test-Path -Path "$path") {
        ...
        for ($r = 0; $r -lt $regpath.count; $r++) {
            ...
            reg add "$reg\0007" /t REG_MULTI_SZ /v "ICMProfile" /d "AyaNeo.icc" /f
            reg add "$reg\0007" /t REG_MULTI_SZ /v "ICMProfileAC" /d "AyaNeo.icc" /f
        ...
        for ($r = 0; $r -lt $regpath.count; $r++) {
            $reg = $regpath[$r]
            reg add "$reg" /t REG_MULTI_SZ /v "ICMProfile" /d "AyaNeo.icc" /f
        }
        ...
        reg add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\ICM\RegisteredProfiles" /t REG_SZ /v "sRGB" /d "AyaNeo.icc" /f
